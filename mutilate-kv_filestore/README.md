Mutilate modified for driving the KV Filestore Microbenchmark
=============================================================

This is a highly modified version of Mutilate built to drive load into 
the KV Filestore microbenchmarks. Its distributed design allows for high request
...
This version of mutilate has been tested on Ubuntu 14.04, 15.04, 15.10 and
16.04 thoroughly.
...
Real deployments of scale out applications often handle the requests of dozens,
...
longer than the time it takes for the server to process one
request.
...
number of outstanding requests, and (3) the server saturates
and experiences queuing delay far before mutilate does. I suggest the
following guidelines:

1. Establish on the order of 100 connections per _server_
thread.
...
====================
    ...
    Basic options:
      ...
      -V, --valuesize=STRING        Length of memcached values (distribution).  
                                      (default=`200')
      --readsize=STRING             Length of read sizes (distribution).
                                      (default=`100')
      --maxreadsize=INT             Maximum length of a read to issue.
                                      (default=`64')

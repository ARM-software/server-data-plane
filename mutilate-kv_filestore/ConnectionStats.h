...
class ConnectionStats {
 ...
 ConnectionStats(bool _sampling = true) :
  ...
  void accumulate(const AgentStats &as) {
    ...
    get_sampler.accumulate(as.get_sampler, as.get_sampler_entries, as.get_sum,
			   as.get_sumsq);
    set_sampler.accumulate(as.set_sampler, as.set_sampler_entries, as.set_sum,
			   as.set_sumsq);
    op_sampler.accumulate(as.op_sampler, as.op_sampler_entries, as.op_sum,
			  as.op_sumsq);
  }
  ...
  void serialize_get_samples(uint64_t *sampler_entries, uint64_t *sampler,
			     uint64_t *sum, uint64_t *sumsq)
  {
    get_sampler.serialize(sampler_entries, sampler, sum, sumsq);
  }

  void serialize_set_samples(uint64_t *sampler_entries, uint64_t *sampler,
			     uint64_t *sum, uint64_t *sumsq)
  {
    set_sampler.serialize(sampler_entries, sampler, sum, sumsq);
  }

  void serialize_op_samples(uint64_t *sampler_entries, uint64_t *sampler,
			     uint64_t *sum, uint64_t *sumsq)
  {
    op_sampler.serialize(sampler_entries, sampler, sum, sumsq);
  }
};

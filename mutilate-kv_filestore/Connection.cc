  ...
  cur_uuid(_uuid)
{
  ...
  if (options.binary) {
    ...
    DIE("Only binary protocol exists!");
  }
  ...
  cur_uuid = cur_uuid << 44; // Make uuid's somewhat unique among connections
                             // so we can debug a bit easier when using
			     // wireshark. The bottom 44 bits is for a
			     // montonically increasing counter to track all
			     // transactions
}
...
void Connection::reset() {
  ...
  assert(op_map.size() == 0);
  evtimer_del(timer);
  read_state = IDLE;
  write_state = INIT_WRITE;
  stats = ConnectionStats(stats.sampling);
  cur_uuid = cur_uuid & 0xfffff00000000000;
}
...
void Connection::start_loading() {
  ...
  for (int i = 0; i < LOADER_CHUNK; i++) {
    ...
    string keypart = keygen->generate(loader_issued);
    string keystr = keypart + port;
    strcpy(key, keystr.c_str());
    issue_create(key, &random_char[index], valuesize->generate());
    loader_issued++;
  }
}
...
void Connection::issue_something(double now) {
  ...
  string keypart = keygen->generate(lrand48() % options.records);
  string keystr = keypart + port; // Key is the id and the port of the server we
                                  // are talking to, that way replication doesn't do wacky things. 
  strcpy(key, keystr.c_str());
  int readlen = (int)readsize->generate() % options.maxreadsize;

  if (drand48() < options.update) {
    int index = lrand48() % (1024 * 1024);
    issue_write(key, &random_char[index], valuesize->generate(), now);
  } else {
    issue_read(key, readlen, now);
  }
}

/**
 * Issue a read request to the server.
 */
void Connection::issue_read(const char* key, uint64_t length, double now) {
  Operation op;
  int l;
  ...
  if (read_state == IDLE) read_state = WAITING_FOR_READ;
  l = prot->read_request(key, length, cur_uuid);
  cur_uuid++;
 ...
 */
void Connection::issue_write(const char* key, const char* value, int length,
                             double now) {
  Operation op;
  int l;
  ...
  if (read_state == IDLE) read_state = WAITING_FOR_WRITE;
  l = prot->write_request(key, value, length, cur_uuid);
  cur_uuid++;
  if (read_state != LOADING) stats.tx_bytes += l;
}

/**
 * Issue a create request to the server
 */
void Connection::issue_create(const char* key, const char* value, int length,
			      double now) {
  Operation op;
  int l;
  ...
  if (read_state == IDLE) read_state = WAITING_FOR_CREATE;
  l = prot->create_request(key, value, length, cur_uuid);
  cur_uuid++;
  if (read_state != LOADING) stats.tx_bytes += l;
}
 ...
 */
void Connection::pop_op(uint64_t key) {
  assert(op_map.size() > 0);

  auto itr = op_map.find(key);
  if (itr != op_map.end()) op_map.erase(itr);
 ...
 * server.
 */
void Connection::finish_op(Operation *op, uint64_t uuid) {
  double now;
#if USE_CACHED_TIME
  struct timeval now_tv;
  event_base_gettimeofday_cached(base, &now_tv);
  ...
  switch (op->type) {
  case Operation::READ: stats.log_get(*op); break;
  case Operation::WRITE: stats.log_set(*op); break;
  default: DIE("Not implemented.");
  }
...
void Connection::drive_write_machine(double now) {
  ...
  while (1) {
    ...
    case ISSUING:
      if (op_map.size() >= (size_t) options.depth) {
    ...
    case WAITING_FOR_OPQ:
      if (op_map.size() >= (size_t) options.depth) return;
      write_state = ISSUING;
      break;
...
void Connection::read_callback() {
  ...
  bool done, full_read;
  uint64_t uuid;
  std::map<uint64_t, Operation>::iterator itr = op_map.end();

  if (op_map.size() == 0) V("Spurious read callback.");
  ...
  while (1) {
    ...
    case WAITING_FOR_READ:
      assert(op_map.size() > 0);
      full_read = prot->handle_response(input, done, uuid);
      if (!full_read) {
        return;
      } else if (done) {
	itr = op_map.find(uuid);
	if (itr != op_map.end()) finish_op(&(itr->second), uuid); // sets read_state = IDLE
	else D("Got a ghost read response!\n");
      }
      break;

    case WAITING_FOR_WRITE:
      assert(op_map.size() > 0);
      if (!prot->handle_response(input, done, uuid)) return;
      itr = op_map.find(uuid);
      if (itr != op_map.end()) finish_op(&(itr->second), uuid);
      else D("Got a ghost write response!\n");
      break;

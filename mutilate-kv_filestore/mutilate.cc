...
void do_mutilate(const vector<string> &servers, options_t &options,
                 ConnectionStats &stats, int tid, bool master = true
#ifdef HAVE_LIBZMQ
, zmq::socket_t* socket = NULL
#endif
);
...
void agent() {
  ...
  while (true) {
    ...
    *((int *) num.data()) = args.threads_arg * args.lambda_mul_arg;
    ...
    stats.serialize_get_samples(&as.get_sampler_entries, as.get_sampler,
				&as.get_sum, &as.get_sumsq);
    stats.serialize_set_samples(&as.set_sampler_entries, as.set_sampler,
				&as.set_sum, &as.set_sumsq);
    stats.serialize_op_samples(&as.op_sampler_entries, as.op_sampler,
				&as.op_sum, &as.op_sumsq);
...
void finish_agent(ConnectionStats &stats) {
  for (auto s: agent_sockets) {
    V("MASTER REQUEST AGENT STATS");
    s_send(*s, "stats");
    V("MASTER REQUEST AGENT STATS SENT");

    AgentStats as;
    zmq::message_t message;

    V("MASTER WAITING TO RECV");
    s->recv(&message);
    V("MASTER RECV AGENT STATS");
    memcpy(&as, message.data(), sizeof(as));
    V("Agents stats: %d %d %d %d %d", sizeof(as), as.get_sampler_entries, as.set_sampler_entries, as.op_sampler_entries);
    stats.accumulate(as);
  }
}
...
, zmq::socket_t* socket
  ...
  if (options.threads > 1) {
    ...
    do_mutilate(servers, options, stats, 0, true
#ifdef HAVE_LIBZMQ
, socket
#endif
);
...
void* thread_main(void *arg) {
  ...
  do_mutilate(*td->servers, *td->options, *cs, td->tid, td->master
#ifdef HAVE_LIBZMQ
, td->socket
#endif
);
...
void watchdog_cb(evutil_socket_t fd, short what, void *arg)
{
  return;
}

void do_mutilate(const vector<string>& servers, options_t& options,
                 ConnectionStats& stats, int tid, bool master
#ifdef HAVE_LIBZMQ
, zmq::socket_t* socket
#endif
) {
  ...
  uint64_t uuid = ((uint64_t)tid << 10) | 0;
  ...
  watchdog = evtimer_new(base, watchdog_cb, NULL);

  // Main event loop.
  while (1) {
    if (!event_pending(watchdog, EV_TIMEOUT, NULL)) {
      evtimer_add(watchdog, &one_sec);
    }
    event_base_loop(base, loop_flag);
...
void args_to_options(options_t* options) {
  ...
  strcpy(options->readsize, args.readsize_arg);
  options->maxreadsize = args.maxreadsize_arg;

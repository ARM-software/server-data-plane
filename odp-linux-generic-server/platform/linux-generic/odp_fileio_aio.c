 ...
 *      again. Have tried numerous workarounds but
 *      still seem to be hitting unknown corner cases.  Going to
 *      assume this implementation is fundamentally broken for now as the
 *      bug is still hidden.
 */
...
int sweep_fileio_completions(fileio_queue *fioq)
{
	...
	while (io_getevents(aio_ctx, 1, 1, &io_evt, &aio_tmo) > 0) {
		...
		// XXX: Only allow 64 outstanding operations right now while debugging AIO.
		// Unmark the in-flight bitmask
		uint64_t index = (cmd->hash >> ODP_FILEIO_MAXQS) %
				  ODP_FILEIO_SCOREBOARD_MAX;
		...
		if (fioq->in_flight_scoreboard[index] == iocb_cmd
			...
			// Hoping renotifications are a benign bug ... confident the mutual exclusion on my end is working
			iocb = (struct iocb*)odp_buffer_addr(iocb_buf);
			memset(iocb, 0, sizeof(struct iocb)); // zero out the iocb in case it rises from the dead.

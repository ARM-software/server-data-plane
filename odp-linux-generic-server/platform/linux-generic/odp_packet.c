...
odp_packet_t packet_alloc(odp_pool_t pool_hdl, uint32_t len, int parse)
{
	...
	if (len == 0) {
		...
		pkt = (odp_packet_t)odp_buffer_alloc_size(pool_hdl, len);
...
void odp_packet_set_len(odp_packet_t pkt, uint32_t len)
{
    packet_set_len(odp_packet_hdr(pkt), len);
}
			   ...
			   uint32_t len, void *dst)
{
    ...
    if (odp_is_chained_buffer(_odp_packet_to_buffer(pkt))) {
        return odp_chained_buffer_copyout(_odp_packet_to_buffer(pkt),
                                          offset, dst, len);
    }
	...
	while (len > 0) {
        ...
        assert(pkt_hdr->headroom == 0);
        assert(((offset + pkt_hdr->headroom) % pkt_hdr->buf_hdr.segsize) + cpylen <= pkt_hdr->buf_hdr.segsize);
			     ...
			     uint32_t len, const void *src)
{
	...
	if (odp_is_chained_buffer(_odp_packet_to_buffer(pkt))) {
		return odp_chained_buffer_copyin(_odp_packet_to_buffer(pkt),
						 offset, src, len);
	}

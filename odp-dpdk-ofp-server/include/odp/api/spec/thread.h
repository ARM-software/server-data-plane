...
typedef enum odp_thread_type_e {
	 ...
	 * driven.
	 */
	ODP_THREAD_CONTROL = 1,

	/**
	 * IO Worker thread
	 *
	 * These threads process the incoming IO and offload communicating
	 * with the device or Linux kernel IO for the worker threads. IO
	 * workers will also participate in general application processing as 
	 * well.  This is an active research question of whether we should
	 * strictly partition the system that needs to access IO.
	 */
	ODP_THREAD_IO_WORKER = 2,
} odp_thread_type_t;

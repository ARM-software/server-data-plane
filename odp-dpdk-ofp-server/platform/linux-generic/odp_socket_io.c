...
odp_sockio_t odp_sockio_create_listener(int port, char* interface)
{
	...
	if (error != 0) {
		if (error != EAI_SYSTEM)
			ODP_ERR("getaddrinfo(): %s\n", gai_strerror(error));
		else
			ODP_ERR("getaddrinfo() hit unknown error.\n");
	}

	// Set up the socket
	sfd = socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol);
	if (sfd == -1) {
		ODP_ERR("failed opening a listening socket.\n");
		return ODP_SOCKIO_INVALID;
	}

	if ((flags = fcntl(sfd, F_GETFL, 0)) < 0 ||
	    fcntl(sfd, F_SETFL, flags | O_NONBLOCK) < 0) {
		ODP_ERR("setting O_NONBLOCK failed for listening socket\n");
		close(sfd);
		return ODP_SOCKIO_INVALID;
	}

	// Set up all the socket options for this socket descriptor
	error = setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, (void *)&flags,
			   sizeof(flags));
	if (error != 0)
		ODP_ERR("setsockopt error.\n");
	error = setsockopt(sfd, SOL_SOCKET, SO_KEEPALIVE, (void *)&flags,
			     sizeof(flags));
	if (error != 0)
		ODP_ERR("setsockopt error.\n");
	error = setsockopt(sfd, SOL_SOCKET, SO_LINGER, (void *)&flags,
			   sizeof(flags));
	if (error != 0)
		ODP_ERR("setsockopt error.\n");
	...
	error = bind(sfd, ai->ai_addr, ai->ai_addrlen);
	if (error != 0) {
		ODP_ERR("bind error on socket.\n");
		return ODP_SOCKIO_INVALID;
	}
	error = listen(sfd, ODP_SOCKET_MAX_BACKLOG);
	if (error != 0) {
		close(sfd);
		freeaddrinfo(ai);
				      ...
				      odp_sockio_type_t type, uint32_t flags)
{
	...
	sockio_entry->s.sfd.buf_size = pool_entry->s.seg_size;
...
odp_sockio_t odp_sockio_open(uint32_t sockfd, odp_pool_t pool_hdl)
{
	...
	res = fcntl(sfd, F_SETFL, fcntl(sfd, F_GETFL)); //| O_NONBLOCK);
	if (res < 0) {
		ODP_ERR("Could not set socket as non-blocking\n");
		return ODP_SOCKIO_INVALID;
	}

	return odp_sockio_create_queues(sfd, pool_hdl, type,
					ODP_SOCKIO_CREATE_ALL_QUEUES);
}

// Don't give socket to epoll until we're ready -- ie, set up packetizers,
// classifiers...
...
static int _sockio_recv(sockio_entry_t *sockio_entry, odp_packet_t pkt_table[], unsigned len)
{
	...
	if (ret < 0) {
		ODP_DBG("sfd %d hit error on pushsend()\n", sfd);
		return -1;
	}
	...
	for (pkts = 0; pkts < (int)len;) {
		...
		pkt_buf = odp_packet_head(pkt); // Get start of buffer, should
						// not be using
						// headroom/tailroom
		recv_bytes = recvfrom(sfd, pkt_buf, max_frame_len, MSG_DONTWAIT,
				      NULL, NULL);
...
int odp_sockio_sendpkt(odp_packet_t pkt, sockio_entry_t *sockio_entry)
{
	...
	uint32_t offset = 0;
	uint32_t sent = 0;
	uint32_t seg_node_idx;
	uint32_t segment_size = hdr->segsize * ODP_BUFFER_MAX_SEG;
	int ret = 0;
	int i;
	...
	seg_node_idx = offset / segment_size;
	for (i = 0; i < (int)seg_node_idx; i++) {
		seg = seg->next_segs;
	}
	curoff = offset - seg_node_idx * segment_size;

	while (to_send) {
		mapaddr =
		    (uint8_t *)buffer_map(seg, curoff, &seglen, seg->size);
		cpylen = to_send > seglen ? seglen : to_send;
		sent = 0;
		while (sent < cpylen) {
			ret = send(sfd, mapaddr + sent, cpylen - sent, flags);
			if (ret > 0)
				sent += ret;
			else if (ret == -1) {
				if (errno != EAGAIN && errno != EWOULDBLOCK) {
...
int sockin_poll(uint32_t sockio_idx)
{
	...
	// Reset epoll descriptor
	sched_fn->reset_sockio(entry->s.index,
			       entry->s.sfd.sockfd,
			       entry->s.epoll_events);
	return pkts;
}
...
int _listen_sockin_accept(sockio_entry_t *entry, odp_event_t *evt_tbl, int num)
{
	...
	for (int i = 0; i < num; i++) {
		...
		sfd = accept(entry->s.sfd.sockfd, NULL, NULL);
		if (sfd < 0) {
			break;
		} else {

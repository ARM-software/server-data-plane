...
int sched_cb_pktin_poll(int pktio_index, int num_queue, int index[])
{
	...
	for (idx = 0; idx < num_queue; idx++) {
		...
		// XXX: Assume all pkts will go into OFP for now XXX
		for (pkts = 0; pkts < num; pkts++) {
			in_pkt = _odp_packet_from_buffer(odp_hdr_to_buf(hdr_tbl[pkts]));
			ofp_packet_input(in_pkt, queue, ofp_eth_vlan_processing);
		}
		// XXX: TODO: Solution needed to allow both raw packet io and OFP XXX
	}
			    ...
			    const odp_pktout_queue_param_t *param)
{
	...
	if (mode == ODP_PKTOUT_MODE_QUEUE) {
		for (i = 0; i < num_queues; i++) {
			...
			qentry->s.enqueue       = queue_ioout_enq;
			qentry->s.dequeue       = pktout_dequeue;
			qentry->s.enqueue_multi = queue_ioout_enq_multi;

...

{
	...
	if (entry->input_pkt_buffer != ODP_PACKET_INVALID &&
		...
		ret = odp_append_fragment_to_chained_buffer(
		    _odp_packet_to_buffer(input_pkt), odp_packet_len(input_pkt),
		    _odp_packet_to_buffer(entry->input_pkt_buffer),
		    odp_packet_len(entry->input_pkt_buffer));

		if (odp_unlikely(ret == -1)) {
			printf("Failed to append buffer to input pkt\n");
			assert(0);
		}

		// Set ultimate length of the packet
		odp_packet_set_len(entry->input_pkt_buffer,
				   odp_packet_len(entry->input_pkt_buffer) +
				       in_pkt_len);
	...
	for (i = 0; i < (int)len && pkt_offset < (int)buf_len; i++) {
		...
		if ((pkt_offset + pkt_hdr_size) <= buf_len) {
			...
			if (odp_unlikely(pkt_size > 1048576)) {
				dbg_hdr = odp_buf_to_hdr(buf_hdl);
				ODP_ERR("Got bad data for packet size %d, %x\n",
					stream_pkt_size, dbg_hdr->segcount);
				assert(0);
				return -1;
			}
			...
			if (pkt_offset + pkt_size <= buf_len) {
				...
				if (odp_unlikely(
					odp_sizeof_chained_buffer(odp_buffer_from_event(odp_packet_to_event(out_pkt))) < pkt_size)) {
				...
				copied = odp_chained_buffer_copydata(
				    _odp_packet_to_buffer(out_pkt), 0, buf_hdl,
				    pkt_offset, pkt_size);
				if (odp_unlikely(copied == -1)) {
	...
	if (pkt_offset == (int)buf_len) {
		odp_chained_buffer_free(
		    _odp_packet_to_buffer(entry->input_pkt_buffer));
		entry->inbuf_offset = 0;
		entry->input_pkt_buffer = ODP_PACKET_INVALID;
	} else {

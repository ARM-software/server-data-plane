...
#include <odp_linux_helper_internal.h>

static struct {
	int proc; /* true when process mode is required, false otherwise */
} helper_options;

static void *run_start_routine(void *arg)
{
	linux_thr_params_t *thr_params = arg;

	/* ODP thread local init */
	if (odp_init_local(thr_params->instance, thr_params->thr_type)) {
		ODP_ERR("Local init failed for internal service thread\n");
		return NULL;
	}
	...
	if (ret < 0)
		ODP_ERR("Local term failed for internal service thread\n");
	else if (ret == 0 && odp_term_global(thr_params->instance))
		ODP_ERR("Global term failed for internal service thread\n");
			 ...
			 const linux_thr_params_t *thr_params)
{
	...
	memset(pthread_tbl, 0, num * sizeof(linux_pthread_t));
			 ...
			 const linux_thr_params_t *thr_params)
{
	...
	memset(proc_tbl, 0, num * sizeof(linux_process_t));
		       ...
		       const linux_thr_params_t *thr_params)
{
	...
	return linux_process_fork_n(proc, &mask, thr_params);
}
...
int linux_process_wait_n(linux_process_t *proc_tbl, int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (WIFEXITED(status) && WEXITSTATUS(status) != EXIT_SUCCESS) {
			ODP_ERR("Child exit status:%d (pid:%d)\n",
		...
		if (WIFSIGNALED(status)) {
			...
			ODP_ERR("Child term signo:%d - %s (pid:%d)\n",
...
static void *odpthread_run_start_routine(void *arg)
{
	int status;
	int ret;
	odpthread_params_t *thr_params;

	odpthread_start_args_t *start_args = arg;
	...
	ODP_DBG("helper: ODP %s thread started as linux %s. (pid=%d)\n",
 ...
 */
static int linux_process_create(odpthread_t *thread_tbl,
				int cpu,
 ...
 */
static int linux_thread_create(odpthread_t *thread_tbl,
			       int cpu,
			       const odpthread_params_t *thr_params)
{
	int ret;
		      ...
		      const odpthread_params_t *thr_params)
{
	...
	memset(thread_tbl, 0, num * sizeof(odpthread_t));

	cpu_count = odp_cpu_count();

	if (num < 1 || num > cpu_count) {
		ODP_ERR("Invalid number of odpthreads:%d"
			 " (%d cores available)\n",
			 num, cpu_count);
		return -1;
	}
	...
	for (i = 0; i < num; i++) {
		if (!helper_options.proc) {
			if (linux_thread_create(&thread_tbl[i],
						     cpu,
						     thr_params))
				break;
		 } else {
			if (linux_process_create(&thread_tbl[i],
						      cpu,
						      thr_params))
				break;
		}
	...
	do {
		...
		case ODPTHREAD_PTHREAD:
			...
			if (ret != 0) {
				...
				terminated++;
				if (thread_ret != NULL) {
					ODP_ERR("Bad exit status cpu #%d %p\n",
		...
		case ODPTHREAD_NOT_STARTED:
			ODP_DBG("No join done on not started ODPthread.\n");
			break;
		default:
			ODP_DBG("Invalid case statement value!\n");
			break;
		}
		  ...
		  const struct option *caller_longopts)
{
	...
	if (merge_getopt_options(caller_shortopts, helper_short_options,

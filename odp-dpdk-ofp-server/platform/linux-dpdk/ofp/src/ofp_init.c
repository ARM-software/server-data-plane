...
int ofp_init_global(odp_instance_t instance, ofp_init_global_t *params)
{
	...
	num_pkt_queues = getenv("OFP_NUM_PKTIO_QUEUES");
	pktin_param.num_queues = atoi(num_pkt_queues);
	...
	if (!linux_pthread_create(&shm->nl_thread,
				  &cpumask,
				  &thr_params)) {
...
int ofp_term_global(void)
{
	...
	if (shm->nl_thread_is_running) {
		linux_pthread_join(&shm->nl_thread, 1);
		shm->nl_thread_is_running = 0;
	}
#endif /* SP */
...
#ifdef SP
		linux_pthread_join(ifnet->rx_tbl, 1);
		linux_pthread_join(ifnet->tx_tbl, 1);
		close(ifnet->fd);
		ifnet->fd = -1;
#endif /*SP*/

		    ...
		    const odp_platform_init_t *platform_params)
{
	...
	*instance = INSTANCE_ID;
	...
	// Initialize OFP data-structures here for entire application
	// Start up CLI for OFP here.
	// XXX: Hard code in some defaults into this function for ODP, for
	//      now.
	ofp_init_global_t ofp_init_params;
	memset(&ofp_init_params, 0, sizeof(ofp_init_params));
	ofp_init_params.if_count = 1;
	ofp_init_params.linux_core_id = 0;
	ofp_init_params.if_names = (char**)malloc(sizeof(char*));
	const char *DEFAULT_DPDK_PKTIO = "0"; // Hard code to the first dpdk pktio source
	ofp_init_params.if_names[0] = (char*)malloc(strlen(DEFAULT_DPDK_PKTIO)+1);
	// Cast away const because OFP doesn't use it...
	memcpy(ofp_init_params.if_names[0], (const void*)DEFAULT_DPDK_PKTIO,
	       strlen(DEFAULT_DPDK_PKTIO)+1);

	if (ofp_init_global(*instance, &ofp_init_params)) {
		ODP_ERR("OFP init failed.\n");
		goto init_failed;
	}

	// XXX: Hard code in the defaults into this function and also a default
	// conf file.  Use env variables like DPDK to set params.
	const char *DEFAULT_OFP_CONF_FILE = "/configs/odp_conf.conf";
	if (ofp_start_cli_thread(*instance, ofp_init_params.linux_core_id,
				 (char*)(uintptr_t)DEFAULT_OFP_CONF_FILE)) {
		ODP_ERR("OFP CLI init failed.\n");
		goto init_failed;
	}
	stage = OFP_INIT;
...
int _odp_term_local(enum init_stage stage)
{
	...
	case OFP_INIT:
		if (ofp_term_local()) {
			ODP_ERR("OFP init local term failed.\n");
			rc = -1;
		}
		/* Fall through */

...
static int schedule_sockio_init(uint32_t sockio_idx, int fd)
{
	odp_buffer_t buf;
	sched_cmd_t *sched_cmd;
	buf = odp_buffer_alloc(sched->pool);
	...
	sched_cmd->in_queue_idx = queue_to_id(sockio_inq_getdef_by_index(sockio_idx));

	sched->sockio[sockio_idx].cmd_ev = odp_buffer_to_event(buf);
	sched->sockio[sockio_idx].fd = fd;

	sched->sockio[sockio_idx].pri_queue = pri_set_queue(sockio_idx,
							   ODP_SCHED_PRIO_DEFAULT);
	sched->sockio[sockio_idx].prio = ODP_SCHED_PRIO_DEFAULT;
	sched->sockio[sockio_idx].scheduled = 0;
	sched->sockio[sockio_idx].pending = 0;

	// XXX: Assume this function is called once on initialization of a
	// sockio, and that is protected by locks
	odp_ticketlock_init(&sched->sockio[sockio_idx].ev_lock);

	return 0;
}

static int schedule_sockio_start(uint32_t sockio_idx, int fd UNUSED)
{
	/* This should not change */
	// ODP_ASSERT(fd == sched->sockio[sockio_idx].fd);

	odp_queue_t pri_queue = sched->sockio[sockio_idx].pri_queue;
	odp_event_t cmd_ev    = sched->sockio[sockio_idx].cmd_ev;

	// XXX: protect access to this operation with the ev_lock
	//      check if we are already scheduled.
	odp_ticketlock_lock(&sched->sockio[sockio_idx].ev_lock);
	int ret = 0;
	if (sched->sockio[sockio_idx].scheduled == 0) {
		sched->sockio[sockio_idx].scheduled = 1;
		sched_local.ignore_ordered_context = 1;
		ret = odp_queue_enq(pri_queue, cmd_ev);
	} else {
		sched->sockio[sockio_idx].pending = 1;
	}
	odp_ticketlock_unlock(&sched->sockio[sockio_idx].ev_lock);

	return ret;
}

// This function is meant to be used by the scheduler only to take the
// scheduling event off the priority queues so other async events like
// packet receipt can be used to put them back on.
static int deschedule_sockio_start(uint32_t sockio_idx)
{
	ODP_ASSERT(sockio_idx < ODP_CONFIG_SOCKIO_ENTRIES);

	odp_ticketlock_lock(&sched->sockio[sockio_idx].ev_lock);
	int ret = 0;
	if (sched->sockio[sockio_idx].pending == 1) {
		odp_queue_t pri_queue = sched->sockio[sockio_idx].pri_queue;
		odp_event_t cmd_ev = sched->sockio[sockio_idx].cmd_ev;
...
static int schedule_sockio_reset(uint32_t sockio_idx, int fd)
{
	/* This should not change */
	ODP_ASSERT(0);
	ODP_ASSERT(fd = sched->sockio[sockio_idx].fd);
		...
		for (j = 0; j < QUEUES_PER_PRIO; j++, id++) {
			 ...
			 */
				...
				num = sched_cb_queue_deq_multi(qi, sched_local.ev_stash,
						               max_deq);
			} else if (sched_cmd->cmd == SCHED_CMD_POLL_SOCKIN) {
				// Suck in messages from sockets
				if (sockin_poll(sched_cmd->sockio_idx)) {
					odp_queue_enq(pri_q, ev);
					// call the sockio start function to get
					// locking semantics as multiple agents
					// can be attempting to reschedule this.
				} else {
					deschedule_sockio_start(sched_cmd->sockio_idx);
				}
				continue; // Find some application events;
			}

			// XXX: Super hack! All timeouts go to OFP.  This would
			// need to be fixed for any real usage. Same with the
			// super hack in pktio for doing stack processing. XXX
			if (num > 0) {
				if (odp_event_type(sched_local.ev_stash[0]) ==
					ODP_EVENT_TIMEOUT) {
					int i = 0;
					for (i = 0; i < num; i++) {
						ofp_timer_handle(sched_local.ev_stash[i]);
					}
					num = 0;
				}
			}

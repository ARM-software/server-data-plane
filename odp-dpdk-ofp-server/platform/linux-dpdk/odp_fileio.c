...
#include <odp_chained_buffer_internal.h>
		...
		if (evt != ODP_EVENT_INVALID) {
			...
			while (bytes < cmd->size) {
				...
				if (cmd->num_iovs > 0) {
					...
					uint32_t iov_len = cur_iov->iov_len;
					//Assumes the buffer is composed of
					//segments all the same length. XXX
...
int create_iov_list_from_buffer(odp_buffer_t buf, struct iovec *iov, size_t buf_size)
{
	...
	int num_segs = odp_chained_buffer_num_segs(buf);

	if (num_segs > MAX_IOVEC) {
	...
	while (to_map > 0) {
		iov[i].iov_base = (char*)(mb->buf_addr);
		iov[i].iov_len = (to_map < mb->data_len) ? to_map : mb->data_len;

		to_map -= (to_map < mb->data_len) ? to_map : mb->data_len;
		mb = mb->next;
	}
	ODP_ASSERT(to_map == 0 );
	return i;
